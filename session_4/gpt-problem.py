...
with open('session_4/input.txt', 'r', encoding='utf-8') as f:
    text = f.read()
...
class GPT(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        # Words or tokens are typically represented as dense vectors in a high-dimensional space. The token embedding table is a lookup table that maps each token in the vocabulary to its corresponding dense vector representation: 
    ...
    def forward(self, idx, targets=None):
        ...
        # pos_emb = pos_emb[None, :, :].expand(B, T, n_embd) # (B, T, C)
